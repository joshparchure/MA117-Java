...
public abstract class Matrix {
    ...
    public String toString() {
        ...
        String stringMatrix = "";
        // Loop over the 2 dimensions of the matrix, adding the elements row by row
        for (int i=0; i < this.iDim; i++){
            // Add the first element of each row outside of the 2nd loop, formatted to 2 dp.
            stringMatrix = stringMatrix + String.format("%.2f",getIJ(i,0));
            for (int j=1; j < this.jDim; j++){
                // Concatenate the string with the (i,j)th entry formatted to 2 dp.
                stringMatrix = stringMatrix + " " + String.format("%.2f",getIJ(i,j));
                // By adding the first element of each row separately, the spaces are added BEFORE each element is added.
                // This way we don't have a space after the last element of each row.
            }
            // Add a new line to the string representation.
            stringMatrix = stringMatrix + "\n";
        }
        // We use the trim method to remove the empty new line added in the last iteration of the nested loop above.
        return stringMatrix.trim();
    }
